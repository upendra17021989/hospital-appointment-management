...
// ── Appointment Card ──────────────────────────────────────────
const AppointmentCard = ({ appt, onPrescription, navigate }) => (
          ...
          {Object.values(vitals).some(Boolean) && (
            <div className="pd-presc-section">
              ...
              <div className="pd-vitals-row">
                ...
                {vitals.pulse  && <div className="pd-vital"><span>Pulse</span>{vitals.bp} bpm</div>}
...
const PatientDetail = () => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await api.put(`/patients/hospital/${id}`, editForm);
      setSaveMsg('Patient details updated successfully!');
      // Refresh patient
      const p = await api.get(`/patients/hospital/${id}`);
      setPatient(p);
  ...
  return (
    <div>
      {/* Back button */}
      <Link to="/patients" className="pd-back-btn">
        ← Back to Patients
      </Link>
      ...
      <div className="pd-tabs">
        {[
          ...
          <Tab
            ...
            onClick={() => { 
              setActiveTab(t.key); 
              if (t.key !== 'edit') setEditMode(false); 
            }}
          />
        ))}
      </div>
        ...
        <div className="pd-tab-content">
          <div className="pd-edit-card card">
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', marginTop: 20 }}>
              <button className="btn btn-secondary" onClick={() => setEditForm(patient || {})}>
                Reset
              </button>
